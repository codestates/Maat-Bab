...
function SignUp({certificationCode, email, setEamil}) {
    ...
    const overlapHandler = () => {
        if(!isEmail(email)){
            ...
            axios.post('http://localhost:80/same-email', {email:email})
        .then((res) => {
            if(res.status === 204){
                alert('사용가능한 이메일입니다')
                setOverlap(true)
               ...
               <h2 className='signup__title'>회원가입</h2>
                <div className='signup__container'>
                   <div className='signup__container__row email'>
                      ...
                      <input onChange={(e) => onChange3(e)} disabled={overlap ? 'disabled' : ''} type='email' className='signup__email' placeholder='이메일'></input>
                      <button onClick={overlapHandler} className='signup__check__email'>중복확인</button>
                    ...
                    <div className='signup__container__row password'>
                        ...
                        <input type='password' onKeyUp={(e) => onChange1(e)} className='signup__password' placeholder='특수문자를 포함해야합니다'></input>
                    </div>
                 <div className='signup__container__row passwordcheck'>
                   ...
                   <div className='signup__container__row username'>
                        ...
                        <input onChange={(e) => onchange4(e)} type='text' className='signup__username' placeholder='사용자 이름'></input>
                    </div>
                </div>
