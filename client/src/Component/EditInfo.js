...
function EditInfo() {
    ...
    useEffect(() => {
        ...
        setManner(newArr)
        axios.get(`http://localhost:80/taste`)
        .then((res) => {
    ...
    useEffect(() => {
    axios.get('http://localhost:80/userinfo/taste')
    .then(res => {
        const myData = res.data;
        const selectedList = foodLists.map(food => {
            if(myData.some((myfood, index) => food.taste_id === myfood.taste_id)){
                return {
                   ...food,
                   selected: true
                }
            }else if(myData.some((myfood, index) => food.taste_id !== myfood.taste_id)){
                return {
                    ...food
                }
            }
    ...
    axios.get(`http://localhost:80/userinfo/etiquette`)
    .then(res => {
    ...
    const infoCorrection = () => {
        if(nickName === '' || passWord === '' || passWordCheck === ''){
            ...
            axios.patch(`http://localhost:80/userinfo`, {name:nickName, password:passWord})
            .then(res => {
                if(res.status === 200){
    ...
    const foodCorrection = () => {
        ...
        axios.patch(`http://localhost:80/userinfo/taste`,{taste_id:filterIdx})
        .then(res => {
            if(res.status === 200){
                alert('수정이 완료되었습니다')
            }else{
    ...
    const addMannerBtn = () => {
        setManner([...manner, {manner_id:manner.length, text:addManner, selected:true}])
        ...
        <div className='edit__background'>
            <div className='edit__content'>
                ...
                <div className='edit__user__box'>
                    ...
                    <input onChange={(e) => onchangeName(e)} type='text' placeholder='이름' className='edit__username__input'></input>
                </div>
                <button onClick={infoCorrection} className='edit__button__done'>수정</button>
                <div className='edit__title__content'>음식 취향</div>
                <div className='edit__preferece__box'>
                    {sumLists.map((el) => {
                        if(el.selected === false){
                            return  <div onClick={() => selectFoodHandler(el.taste_id)} className='edit__food'>{el.name}</div>
                        }else{
                            return <div onClick={() => selectFoodHandler(el.taste_id)} className='edit__food__seleted'>{el.name}</div>
                        }
                    })}
                </div>
                <button onClick={foodCorrection} className='edit__button__done'>선택완료</button>
                <div className='edit__title__content'>식사 예절</div>
                <div className='edit__tablemanner__box'>
                    {sumManner.map((el) => {
                        if(el.selected === false){
                            return <div onClick={() => selectMannerHandler(el.text)} className='edit__manner'>{el.text}</div>
                        }else{
                            return <div onClick={() => selectMannerHandler(el.text)} className='edit__manner__selected'>{el.text}</div>
                        }
                    })}
                </div>
                <div className='edit__add__button'>
                <input onChange={(e) => onchangeAddManner(e)} placeholder='추가할 예절을 입력해주세요' type='text' className='edit__custom__manner__input'></input>
                <button onClick={addMannerBtn} className='edit__custom__add'><FontAwesomeIcon icon={faPlus}/></button>
                </div>
                <button onClick={mannerCorrection} className='edit__button__done'>선택완료</button>
            </div>
